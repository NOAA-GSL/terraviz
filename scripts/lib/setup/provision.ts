 ...
 * which also holds `catalog-schema.sql` — a *generated snapshot* of
 * the fully-migrated catalog schema, written by `npm run
 * db:dump-schema` for reference. Wrangler has no way to know it isn't
 * a migration, so it queues it as one and then fails applying it.
 *
 * This is why `ci.yml` only ever auto-applies `CATALOG_DB`.
 *
 * The ordering consequence, verified against a clean database:
 * **apply `CATALOG_DB` first.** Then every table in the snapshot
 * already exists, so the snapshot fails on its very first statement
 * and changes nothing. Run `FEEDBACK_DB` first on a fresh database
 * and the snapshot applies for real — creating the whole catalog
 * schema out from under the migration tracker, after which every
 * `CATALOG_DB` migration fails on "table already exists".
 *
 * So the failure is expected and harmless *in that order*, and this
 * predicate distinguishes it from a genuine migration failure rather
