...
| `src/services/datasetContours.ts` | Pure isolines over a `LumaSnapshot` — marching squares at one physical threshold → lat/lon polylines → a GeoJSON `MultiLineString` for MapLibre. A cell touching absent data emits nothing (tracing the no-data footprint is the `vmin` mistake in another form), values come from a 256-entry table built *through* `lumaToValue` rather than an inverse of it, saddles resolve by the cell mean, and a line that jumps the antimeridian is cut rather than drawn across the globe (`docs/DATA_ANALYSIS_PLAN.md` §A5) |
