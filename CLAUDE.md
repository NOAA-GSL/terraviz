...
| `src/ui/publisher/components/events/media-suggest.ts` | Events-tab **media suggestions** — pure image-candidate builders for imageless events; first source is the NASA Worldview Snapshots API (keyless, public-domain satellite imagery for the event's bbox + date); the detail pane's "Use as event image" writes the pick through the review endpoint's `edits.imageUrl` |
