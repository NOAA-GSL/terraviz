...
| `src/ui/publisher/components/events/media-suggest.ts` | Events-tab **media suggestions** — image-candidate builders for imageless events: the pure NASA Worldview Snapshots source (keyless, public-domain satellite imagery for the event's bbox + date) and the fetched Wikimedia Commons nearby-photos source (geosearch, kept only when public-domain/CC0 — the stored `image_url` carries no attribution field); the detail pane's "Use as event image" writes the pick through the review endpoint's `edits.imageUrl` |
