...
| `src/ui/analyzeUI.ts` | The Analyze panel (Tools → Analyze) — region picker (whole dataset / current view / a named region via `resolveRegion`), area-weighted statistics over one snapshot of the displayed frame, the palette-coloured histogram, coverage, the quantisation-step caveat, and CSV export. Reads through an injected `AnalyzeSource` rather than a renderer singleton (`docs/DATA_ANALYSIS_PLAN.md` §A3) |
| `src/ui/analyzeCharts.ts` | Hand-rolled SVG for the Analyze panel — the 256-bin histogram painted from the same display LUT the shader samples (square-root height scale, because these fields are far too skewed for a linear one) plus the stat tile. Mirrors `publisher/analytics-charts.ts` without importing across the portal boundary |
| `src/ui/analyzeExport.ts` | CSV serialisation for the Analyze panel — pure `buildCsvText` (header block naming dataset / region / units / quantisation step, the summary, then the full occupied distribution at full precision) plus the browser download trigger |
