...
| `YOUTUBE_API_KEY` | Secret (optional) | YouTube Data API v3 key for the (planned) agency-YouTube media-suggestion source in the events review queue. Acquisition + restriction walkthrough: [`YOUTUBE_API_KEY.md`](YOUTUBE_API_KEY.md). Absent = the source stays off; nothing errors. | Agency-YouTube media suggestions. |
