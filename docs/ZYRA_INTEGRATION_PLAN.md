...
| 2 | **Process** (transform) | Implemented — `decode-grib2`, `extract-variable`, `convert-format`, `reproject` | Decode GRIB2/NetCDF, subset variables, reproject (regional grids → equirectangular; 0–360 global grids → ±180) | `reproject` is allowlisted but awaits the next runner-image bump (on zyra staging, not yet in a tagged release). |
